  ...
  return (
    <div className="w-full">
      ...
      <div className="absolute w-[95vw] z-20 rounded-lg mx-4 p-4 sm:px-4 bg-[#10194D] text-[10px] sm:text-[16px] flex justify-between items-center">
        ...
        <Link to = '/'><p className="cursor-pointer">Curated for you</p></Link>
        <Link to = '/service'><p  className=' cursor-pointer'>Services</p></Link>
        <p onClick={()=>setToggle2(true)} className="cursor-pointer">Raise Capital / Sell Business</p>
        <Link to = '/investor'><p className="cursor-pointer">Intersted in investing</p></Link>
      ...
      {toggle2? 
        <div className="absolute top-0 backdrop-blur-sm z-20 w-full h-[100vh] mb-12 flex justify-center items-center">
        <div className="flex flex-col items-center p-6 bg-slate-800 rounded-xl">
          <p className="text-[26px] ranade mb-8">Raise Capital / Sell Business</p>
          ...
          <button onClick={()=>setToggle2(false)} className="my-3 mt-6 text-[13px] border rounded-2xl px-10 py-2 bg-gradient-to-r from-green-400 to-blue-700">
            Subscribe Now
          </button>
        </div>
      </div>
